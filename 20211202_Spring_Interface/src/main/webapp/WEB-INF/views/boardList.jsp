...
<body>
	...
	<fieldset>
		...
		<p>
			화면에서 요청되는 name값을 변경하여 처리가능<br>
			화면에서는 pw 넘겨줌 하지만 WAS에서는 password 처리한다면<br>
			String password = request.getParameter("pw");
		</p>
		<p>
			화면에서 content와 같이 어떠한 특정한 값이 NN이 허용된다면 default로 처리했음<br>
	...
	<fieldset>
		...
		<ol>
			<li>
				@Controller와 하위의 @GetMapping등을 사용할시<br>
				@Controller @RequestMapping(value="/user/") 선언을 하면 @Controller의 하위의<br>
				메소드의 매핑은 모두 /user/로 요청된 처리만을 담당한다
				<pre>
					http://localhost:8095/test/user/login.do
			...
			<li>
				...
				<form action="./com/min/edu/PramPath.do" method="get">
	...
	<fieldset>
		...
		<p>
			...
			1. &lt;tx:annotation-driven&gt;에 의해서 사용하는 Annotation <br>
			2. aop 설정을 통한 roll-back 상황시 동작되도록 함
		</p>
		<p>
			OCP 구조에 의해서 개발이 진행 <br>
			 &lt;mvc:annotation-driven&gt;에 의해서 MVC가 동작됨
			 IoC에 의해서 Spring Container가 제어를 역전하여 실행 시킴
			 
			 @Service는 Dao를 조합하여 처리함
			 @Service에서 조합되어 있는 메소드를 @Transaction을 처리하여 Commit 혹은 RollBack이 될수 있도록 처리
		</p>
		...
		<form action="./transactionTest.do" method="post">
			...
			<input type="text" name="content" value="Spring 구조상 IoC에 의해서 Transaction을 처리 해야함">
			<input type="submit" value="전송">
		</form>
	</fieldset>
</body>
